...
const getMyChat = TryCatch(async (req, res, next) => {
  ...
  }).populate("participants", "fullName userName profile");

  const transformedConversations = conversations.map(
    ({ _id, groupChat, name, profile, participants }) => {
...
const getMyGroups = TryCatch(async (req, res, next) => {
  const chats = await Conversation.find({
    ...
    groupChat: true,
    creator: req.user._id,
  }).populate("participants", "fullName profile.url");

  const groups = chats.map(
    ({ _id, groupChat, profile, participants, name }) => ({
      _id,
      participants,
      name,
      groupChat,
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  const allNewMembersPromise = participants.map((i) =>
    User.findById(i, "fullName")
  );

  const allNewMembers = await Promise.all(allNewMembersPromise);
  const uniqueMembers = allNewMembers
    .filter((i) => !group.participants.includes(i._id.toString()))
    .map((i) => i._id);
  ...
  emitEvent(req, ALERT, group.participants, {
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  const [user] = await Promise.all([User.findById(req.user._id), chat.save()]);
...
const getChatDetails = TryCatch(async (req, res, next) => {
  if (req.query.populate === "true") {
    ...
    const chat = await Conversation.findById(chatId).populate(
      "participants",
      "fullName profile"
    );
